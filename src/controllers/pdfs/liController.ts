...
import { getNameDocBaseByServicoLI, getNameDocBaseByServicoLP } from '../../services/servicos';

const { buildDocumentoBase } = require("../../report-builder/documento-base-LI/documento-base-builder");
const { generatePdf } = require("../../report-builder/utils/report-utils");

dotenv.config();

export const liReportController = {
  getLIReport: async (req: AuthenticatedUserRequest, res: Response) => {
    ...
    try {
      ...
      const [reportOptions, nameDocumentBaseResponse] = await Promise.all([
        liReportController.getReportOptions(
          Number(empresaId),
          Number(clientId),
          gesIds,
          servicoId
        ),
        getNameDocBaseByServicoLI(Number(empresaId), Number(servicoId)),
      ]);
    ...
    //     docDefinitions.resultadoLP.pdfs.map(async (e: any) => {
    ...
    //   );
      ...
      const basePdfLoadPromise = (async () => {
        const baseUrl = nameDocumentBaseResponse?.dataValues?.base_document_url_li;
      ...
      const [mainPdfBuffer, basePdfDoc] = await Promise.all([
        generatePdfPromise,
        basePdfLoadPromise,
      ]);
