...
import { getNameDocBaseByServicoLP } from '../../services/servicos';

const { buildDocumentoBase } = require("../../report-builder/documento-base-LP/documento-base-builder");
const { generatePdf } = require("../../report-builder/utils/report-utils");

dotenv.config();

export const lpReportController = {
  getLPReport: async (req: AuthenticatedUserRequest, res: Response) => {
    ...
    try {
      ...
      const [reportOptions, nameDocumentBaseResponse] = await Promise.all([
        lpReportController.getReportOptions(
          Number(empresaId),
          Number(clientId),
          gesIds,
          servicoId
        ),
        getNameDocBaseByServicoLP(Number(empresaId), Number(servicoId)),
      ]);
    ...
    //   );
      ...
      const basePdfLoadPromise = (async () => {
        const baseUrl = nameDocumentBaseResponse?.dataValues?.base_document_url_lp;
